---
id: overview
title: Obligation Registry Overview
...
## Overview of the Obligation Registry
...
Everything else — deploying, minting, endorsing, reading the endorsement chain — works the same way you already know from Token Registry / Title Escrow. The next pages walk through each of these for the Obligation Registry specifically.
...
1. [Deployment](./deployment) — deploy your Obligation Registry via CLI or code
2. [Perform Transactions](./transactions) — mint, accept, reject, discharge, transfer, and return to issuer
3. [Fetch Endorsement Chain](./fetch-endorsement-chain) — read a document's full on-chain history, including status changes
