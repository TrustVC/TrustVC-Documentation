...
| **Bill of Exchange (BoE)** *(Beta)* | A written, unconditional order directing a drawee to pay a fixed sum to a payee — issued via the [Obligation Registry](/docs/how-tos/obligation-registry/overview) rather than the classic Token Registry. |
...
- `BILL_OF_EXCHANGE` - Bill of Exchange template *(Beta — see [Obligation Registry Overview](/docs/how-tos/obligation-registry/overview))*
...
:::caution Beta
The Bill of Exchange template is currently in **beta**, as is the [Obligation Registry](/docs/how-tos/obligation-registry/overview) it's issued through. APIs, contract addresses, and behavior may change before the stable release. Use on testnet only and do not rely on this feature in production.
:::
...
Fields on `drawee` / `drawer` (both optional `BillOfExchangeParty` objects): `name`, `address`, `authorisedSignatoryName`, and `signature`. **`signature` must be an inline base64 data URI** (`data:image/png;base64,...`, `image/jpeg`, `image/gif`, `image/webp`, or `image/svg+xml`) — hosted/remote image URLs are rejected by the template for security.

### 4. Create and Issue Your Document

For templates other than Bill of Exchange, you can create and issue a sample document using the [TrustVC Creator (V5 - Mainnet)](https://v5-token-registry.tradetrust.io/creator) / [TrustVC Creator (V5 - Testnet)](https://v5-token-registry.dev.tradetrust.io/creator).
