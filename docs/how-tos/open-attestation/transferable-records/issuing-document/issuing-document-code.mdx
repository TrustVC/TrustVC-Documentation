...
pagination_prev: how-tos/open-attestation/transferable-records/wrapping-document/wrapping-document-code
---
...
Refer to the [pre-requisite](/docs/how-tos/open-attestation/transferable-records/token-registry/token-registry-code) on token registry deployment.
...
const mintRemarks = "This is test mint remark";
const encryptionKey = "testEncryptionKey";

const encryptedMintRemark = encrypt(mintRemark, encryptionKey);
const unconnectedWallet = new Wallet(privateKey);
const provider = ethers.getDefaultProvider("sepolia");
const wallet = unconnectedWallet.connect(provider);

const connectedRegistry = TradeTrustToken__factory.connect(tokenRegistryAddress, wallet);
const tx = await connectedRegistry.mint(owner, holder, tokenId, encryptedMintRemark);
...
[Title Transfer actions](/docs/how-tos/transactions)
