...
Every TrustVC document's provenance can be verified and traced back to its creator or issuer. This is achieved by embedding an
...
If you prefer to use your own domain name for the identity, you may skip the steps involving the CLI and instead read the [DNS Configuration Guide](/docs/how-tos/issuer/dns-did#hands-on-creating-a-dns-did).

## Creating Temporary DNS Proof with CLI

:::note Legacy Method
This method uses the legacy OpenAttestation approach. It is recommended to use the legacy CLI tool (tradetrust-cli) to run this command. For new implementations, we recommend adopting the newer document issuance method. Please refer to this guide: Issuing W3C VCs (link to issuing W3C VC).
:::
