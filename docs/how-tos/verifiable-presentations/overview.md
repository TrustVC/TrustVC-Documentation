...
| Requirement | Detail |
|---|---|
| **The holder's existing DID and key pair** | The private key signs the presentation. It must be the DID the credentials were issued to — not a key generated for the occasion, since a new key is a new DID and would match nothing. Either a `did:key` or a published `did:web`; note the CLI can only create the latter, so a `did:key` holder generates the pair with the library. |
